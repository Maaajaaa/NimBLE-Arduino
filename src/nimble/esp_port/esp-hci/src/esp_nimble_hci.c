...
int ble_hci_trans_hs_cmd_tx(uint8_t *cmd)
{
    ...
    *cmd = BLE_HCI_UART_H4_CMD;
    ...
    if (!esp_vhci_host_check_send_available()) {
        ESP_LOGD(TAG2, "Controller not ready to receive packets");
    }
...
static void ble_hci_rx_acl(uint8_t *data, uint16_t len)
{
    ...
    if (!m) {
        ESP_LOGE(TAG2, "%s failed to allocate ACL buffers; increase ACL_BUF_COUNT", __func__);
        return;
    }
    if ((rc = os_mbuf_append(m, data, len)) != 0) {
        ESP_LOGE(TAG2, "%s failed to os_mbuf_append; rc = %d", __func__, rc);
...
static int host_rcv_pkt(uint8_t *data, uint16_t len)
{
    ...
    if (data[0] == BLE_HCI_UART_H4_EVT) {
        ...
        if (totlen > MYNEWT_VAL(BLE_HCI_EVT_BUF_SIZE)) {
            ESP_LOGE(TAG2, "Received HCI data length at host (%d) exceeds maximum configured HCI event buffer size (%d).",
