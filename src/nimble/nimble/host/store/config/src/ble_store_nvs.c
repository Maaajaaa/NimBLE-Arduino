...
get_nvs_peer_record(char *key_string, struct ble_hs_dev_records *p_dev_rec)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS open operation failed");
...
get_nvs_db_attribute(int obj_type, bool empty, void *value, int num_value)
{
    ...
    for (i = 1; i <= max_limit; i++) {
        ...
        if (err == ESP_ERR_NVS_NOT_FOUND) {
            if (empty) {
                ESP_LOGD(TAG2, "Empty NVS index found = %d for obj_type = %d", i, obj_type);
                return i;
            }
        } else if (err == ESP_OK) {
            count++;
            ...
            ESP_LOGE(TAG2, "NVS read operation failed while fetching size !!");
            return -1;
        }
    }
...
ble_nvs_delete_value(int obj_type, int8_t index)
{
    ...
    if (index > get_nvs_max_obj_value(obj_type)) {
        ESP_LOGE(TAG2, "Invalid index provided to delete");
...
ble_nvs_write_key_value(char *key, const void *value, size_t required_size)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS write operation failed !!");
        goto error;
    }
    ...
    err = nvs_commit(nimble_handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS commit operation failed !!");
        goto error;
    }
...
ble_store_nvs_write(int obj_type, const union ble_store_value *val)
{
    ...
    if (write_key_index == -1) {
        ESP_LOGE(TAG2, "NVS operation failed !!");
        return BLE_HS_ESTORE_FAIL;
        ...
        ESP_LOGD(TAG2, "NVS size overflow.");
    ...
    for (i = 1; i <= get_nvs_max_obj_value(obj_type); i++) {
        ...
        if (obj_type != BLE_STORE_OBJ_TYPE_PEER_DEV_REC) {
            ...
            if (err == ESP_ERR_NVS_NOT_FOUND) {
                ...
                ESP_LOGE(TAG2, "NVS read operation failed !!");
                return -1;
            }
#if MYNEWT_VAL(BLE_HOST_BASED_PRIVACY)
        } else {
        ...
        if (obj_type == BLE_STORE_OBJ_TYPE_PEER_DEV_REC) {
            ESP_LOGD(TAG2, "Peer dev records filled from NVS index = %d", i);
        ...
        {
            if (obj_type == BLE_STORE_OBJ_TYPE_CCCD) {
                ESP_LOGD(TAG2, "CCCD in RAM is filled up from NVS index = %d", i);
                memcpy(db_item, &cur.cccd, sizeof(struct ble_store_value_cccd));
                db_item += sizeof(struct ble_store_value_cccd);
                (*db_num)++;
            } else {
                ESP_LOGD(TAG2, "KEY in RAM is filled up from NVS index = %d", i);
...
ble_nvs_restore_sec_keys(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS operation failed for 'our sec'");
        return err;
    }
    ESP_LOGD(TAG2, "ble_store_config_our_secs restored %d bonds", ble_store_config_num_our_secs);

    err = populate_db_from_nvs(BLE_STORE_OBJ_TYPE_PEER_SEC, ble_store_config_peer_secs,
                               &ble_store_config_num_peer_secs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS operation failed for 'peer sec'");
        return err;
    }
    ESP_LOGD(TAG2, "ble_store_config_peer_secs restored %d bonds",
             ble_store_config_num_peer_secs);

    err = populate_db_from_nvs(BLE_STORE_OBJ_TYPE_CCCD, ble_store_config_cccds,
                               &ble_store_config_num_cccds);
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS operation failed for 'CCCD'");
        return err;
    }
    ESP_LOGD(TAG2, "ble_store_config_cccds restored %d bonds",
             ble_store_config_num_cccds);
...
ble_nvs_restore_peer_records(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG2, "NVS operation failed fetching 'Peer Dev Records'");
        return err;
    }

    ble_rpa_set_num_peer_dev_records(ble_store_num_peer_dev_rec);
    ESP_LOGD(TAG2, "peer_dev_rec restored %d records", ble_store_num_peer_dev_rec);
...
int ble_store_config_persist_cccds(void)
{
    ...
    if (nvs_count == -1) {
        ESP_LOGE(TAG2, "NVS operation failed while persisting CCCD");
        return BLE_HS_ESTORE_FAIL;
    }
    ...
    if (nvs_count < ble_store_config_num_cccds) {
        ...
        ESP_LOGD(TAG2, "Persisting CCCD value in NVS...");
        ...
        if (nvs_idx == -1) {
            ESP_LOGE(TAG2, "NVS delete operation failed for CCCD");
            return BLE_HS_ESTORE_FAIL;
        }
        ESP_LOGD(TAG2, "Deleting CCCD, nvs idx = %d", nvs_idx);
...
int ble_store_config_persist_peer_secs(void)
{
    ...
    if (nvs_count == -1) {
        ESP_LOGE(TAG2, "NVS operation failed while persisting peer sec");
        return BLE_HS_ESTORE_FAIL;
    }
    ...
    if (nvs_count < ble_store_config_num_peer_secs) {
        ...
        /* NVS db count less than RAM count, write operation */
        ESP_LOGD(TAG2, "Persisting peer sec value in NVS...");
        ...
        if (nvs_idx == -1) {
            ESP_LOGE(TAG2, "NVS delete operation failed for peer sec");
            return BLE_HS_ESTORE_FAIL;
        }
        ESP_LOGD(TAG2, "Deleting peer sec, nvs idx = %d", nvs_idx);
...
int ble_store_config_persist_our_secs(void)
{
    ...
    if (nvs_count == -1) {
        ESP_LOGE(TAG2, "NVS operation failed while persisting our sec");
        return BLE_HS_ESTORE_FAIL;
    }
    ...
    if (nvs_count < ble_store_config_num_our_secs) {
        ...
        /* NVS db count less than RAM count, write operation */
        ESP_LOGD(TAG2, "Persisting our sec value to NVS...");
        ...
        if (nvs_idx == -1) {
            ESP_LOGE(TAG2, "NVS delete operation failed for our sec");
            return BLE_HS_ESTORE_FAIL;
        }
        ESP_LOGD(TAG2, "Deleting our sec, nvs idx = %d", nvs_idx);
...
int ble_store_persist_peer_records(void)
{
    ...
    if (nvs_count == -1) {
        ESP_LOGE(TAG2, "NVS operation failed while persisting peer_dev_rec");
        return BLE_HS_ESTORE_FAIL;
    }
    ...
    if (nvs_count < ble_store_num_peer_dev_rec) {
        ...
        /* NVS db count less than RAM count, write operation */
        ESP_LOGD(TAG2, "Persisting peer dev record to NVS...");
        ...
        nvs_idx = get_nvs_db_attribute(BLE_STORE_OBJ_TYPE_PEER_DEV_REC, 0,
                                       ...
                                       ble_store_num_peer_dev_rec);
        if (nvs_idx == -1) {
            ESP_LOGE(TAG2, "NVS delete operation failed for peer records");
            return BLE_HS_ESTORE_FAIL;
        }
        ESP_LOGD(TAG2, "Deleting peer record, nvs idx = %d", nvs_idx);
...
void ble_store_config_conf_init(void)
{
    ...
    if (err != 0) {
        ESP_LOGE(TAG2, "NVS operation failed, can't retrieve the bonding info");
    }
#if MYNEWT_VAL(BLE_HOST_BASED_PRIVACY)
    err = ble_nvs_restore_peer_records();
    if (err != 0) {
        ESP_LOGE(TAG2, "NVS operation failed, can't retrieve the peer records");
    }
#endif
}
